...
task("dev-html", (done) => {
  ...
  // src([options.paths.src.base + "/views/**/dashboards/*.html"])
  //   .pipe(fileinclude({ prefix: "@" }))
...
task("build-html", (done) => {
  ...
  // src([options.paths.src.base + "/views/**/admin-dashboards/*.html"])
  //   .pipe(fileinclude({ prefix: "@" }))
  //   .pipe(dest(options.paths.build.base));
  done();
});
...
task("dev-styles", () => {
  ...
  return src(options.paths.src.css + "/app.scss")
...
task("dev-copy-js-plugins", function () {
...
task("dev-copy-css-plugins", function () {
  return src([options.paths.src.css + "/plugins/**/*.css"]).pipe(
    dest(options.paths.dist.css)
  );
});

task("build-copy-css-plugins", function () {
  return src([options.paths.src.css + "/plugins/**/*.css"]).pipe(
...
task("build-scripts", () => {
  return (
    src([options.paths.src.js + "/main.js"])
      .pipe(concat({ path: "scripts.js" }))
      //    .pipe(uglify())
...
task(
  ...
  series(
    ...
    "build-styles",
    "build-scripts",
    "build-copy-css-plugins",
    "build-copy-js-plugins",
    "build-imgs",
    (done) => {
      console.log(
        "\n\t" + logSymbols.info,
